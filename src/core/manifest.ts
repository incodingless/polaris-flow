...
function getLanguageRoots(lang: SkillLanguage): string[] {
  if (lang === 'zh') {
    return ['zh', 'skills-zh'];
  }
  return ['en', 'skills'];
...
async function collectSharedRules(
  ...
  for (const langRoot of getLanguageRoots(lang)) {
    for (const fileName of langFiles) {
      const candidate = path.join(assetsDir, langRoot, fileName);
...
export function languageSkillsDirToLang(languageSkillsDir: string): SkillLanguage {
  return languageSkillsDir === 'skills-zh' ? 'zh' : 'en';
}

/** init/update 使用的语言 skills 源目录名（与 copyPolarisSkillsForPlatform 一致） */
export function getLanguageSkillsDir(lang: SkillLanguage): string {
