...
感谢你帮助改进 Polaris。这份指南偏实操：说明如何配置项目、准备改动、维护分支、提交 PR，以及如何更新 Skill、shell 脚本等 Polaris 特有资产。
...
脚本位于 `assets/skills/polaris/scripts/`，必须兼容 macOS、Linux 和 Windows Git Bash。
...
- 新增脚本必须加入 `test/ts/polaris-scripts.test.ts` 的 `beforeEach` 拷贝列表。
...
polaris-yaml-validate.sh <- polaris-guard.sh (preflight 阶段)
polaris-handoff.sh <- polaris-state.sh (写入 handoff_context/handoff_hash)
```
...
修改 `.polaris.yaml` 状态文件字段时，需要同步三处：

1. `assets/skills/polaris/scripts/polaris-state.sh`：`cmd_set` 白名单与 enum 校验。
2. `assets/skills/polaris/scripts/polaris-yaml-validate.sh`：schema 校验与 `KNOWN_KEYS`。
3. `test/ts/polaris-scripts.test.ts`：测试中的 YAML 示例与断言。
