...
日期：2026-09-19（A 案实施：2026-09-20）
状态：**A 案已实施** —— D12–D16 全部已决并落地，执行记录见 §八；C 案仍立为后续独立里程碑
...
## 六、待决 —— ✅ 全部已决（用户 2026-09-20）

| # | 问题 | 结论 |
| --- | --- | --- |
| **D12** | 取 A / B / C | ✅ **A**。C 立为后续独立里程碑；B 已被 M2 的 Task 1 覆盖 |
| **D13** | prototype 与 debug 两张表偏移一位，怎么修 | ✅ **按 A 结构性消除**（表只剩「同名映射 + 例外显式登记」）+ 改 `state-next.test.ts` 断言，**单独一个提交**（`b7799f1`）便于回退 |
| **D14** | `state.yaml.phase` 最终去留 | ✅ **本轮不动**，保留为非权威镜像。它的去留依赖 C（「原语双写」还是「删字段」） |
| **D15** | `idle` 是否入 `phases` 表 | ✅ **不入表**（它是「无阶段」空值而非阶段，不占进度分母）；但 `isWritablePhase` 白名单**必须接受它**，否则存量数据与 `initPatches` 默认值会被判非法 |
| **D16** | `TaskPhase` 类型 | ✅ **退化为 `string`** + 注释指向表；枚举正确性交给守卫测试与原语校验 |
...
## 八、执行记录（A 案，2026-09-20）

**状态：A 案（G1–G5）全部落地**，5 笔提交，与 M2/M3 解耦、可独立回退。

| 提交 | 内容 |
| --- | --- |
| `a94417a` | Step 1：阶段表新增 `skill` 字段 + `skillForPhase` / `normalizePhase` / `isWritablePhase` / `writablePhaseList`；护栏测试改向（纯新增，无行为变更） |
| `b7799f1` | Step 2：**D13** —— 删 `state-next` 的两张偏移表，改从阶段表派生（**行为变更，单独立提交**） |
| `7b7f7c5` | Step 3：**G3** —— 两个写原语加白名单校验 + `--force-phase` + `overrides.log` 留痕 |
| `42b9876` | Step 4：**G4/D16** —— 模板与 policy 去枚举、`TaskPhase` 退化、模板镜像加守卫 |
| 本提交 | Step 5：设计文档状态与执行记录、CHANGELOG |

### 目标达成情况

| 目标 | 落点 |
| --- | --- |
| **G1 一个枚举** | `task-kind-layout.ts` 的 `phases` 是唯一可写、可改、可测的集合。`state-next` 的两张表已删（且有测试**断言这三个名字不再导出**，防止复活） |
| **G2 一条约定** | 「游标 = 接下来要执行的阶段 ⇒ 同名映射」写在阶段表 doc 注释、`auto-transition.md`、以及 `task-kind-phases.test.ts` 的可执行语义表三处，说法一致 |
| **G3 写入时暴露** | `phase-validation.ts` + 两个原语；非法值 exit 3 且**不改盘**，报错打印合法集合；`--force-phase` 是唯一绕过口，且**留痕** |
| **G4 文档不自持枚举** | 见下「比设计文档多修的两处」 |
| **G5 不新增转移表** | 只加了 `skill` 一个字段与一个派生函数；例外只有 `null` 一档，没有第二张图 |

### 与设计文档的偏离

1. **`workflow-template.yaml` 实为 5 处枚举，设计文档只列了 2 处**。设计 §1.2 只标了 coding（`:16`）与 debug（`:68`），遗漏同文件里 requirement ×2（`:31`/`:38`）与 prototype ×1（`:54`）。后三处**取值是对的**，所以没被任何测试或人工审计发现 —— 但"正确的枚举"同样是第二份真相，一样会漂移。已一并收敛，并把守卫从「断言不含 triage/prescribe/delivery」改成「**`phase:` 行的注释里不许出现 `|` 分隔的枚举**」—— 只查具体错值会漏掉这一类。
2. **`append-active --phase` 也纳入校验**（设计 §4.2 只提了 `update-active`）。建 entry 时的初始 phase 写错同样致命，且它写的就是游标。
3. **`complete-phase --next-phase` 也纳入校验**：它会前移顶层 phase，与 `--phase` 同一风险级别。
4. **testcase 族的「无自动衔接」需要在表上登记**。设计写「testing 族表为空」但未说原因；实查后原因是**技能目录名与阶段码对不上**（技能是 `case`/`acceptance`，阶段码是 `discovery`/`draft`/`refine`/`ship`）。若不登记，同名映射会产出 `polaris:testing:draft` 这种**不存在的技能名**。已在表上用 `skill: null` + 注释登记。
5. **`state-next.test.ts` 刻意删掉了映射表断言**，改为守卫「不许把表搬回来」+ 集成行为 + D13 回归。理由：把映射表在测试里再抄一份，正是本次归一要消除的第二份真相。
6. **顺带给 `docs/specs/2026-09-16-debug-workflow-design.md` 加了作废标注**（设计 §4.3 未列）。它自称「已定稿（已实现）· 无待决」却描述六阶段，先读它的人必然被误导 —— M2 期间我自己就踩过这个坑（把它当真相、被用户纠正）。

### 实施期踩的坑

1. **模板镜像守卫的第一版解析器抓错了行**：`parsePhaseCodes` 取「第一条 `# 可选值:`」，而模板里 `language` 字段也有一条（`en-英文 | zh-中文`），于是解析出 `en` 这种假值。改为「**顶格 `phase:` 上方最近一条**注释」。
2. **D13 的对照验证第一遍是无效的**：我在旧提交的 worktree 里跑 `state next`，但把**新构建的 dist 拷了进去**，两边输出当然一样。重跑时发现旧 worktree 的 `node build.js` 因缺 `dashboard/node_modules` 在 vite 步失败（tsc 已产出 dist，可用），并用 `grep -c PHASE_TO_SKILL dist/...` 确认跑的是旧代码，才拿到真实对照。

### 未做（登记为后续）

- **C 案（收权）**：技能不再自己写 phase，改由统一的阶段推进原语在**一处**完成「完成当前 + 前移游标 + 同步 state.yaml」。它要改 12 个技能文件、34 处调用，是 A 案之后的独立里程碑。D14（`state.yaml.phase` 去留）随之决定。
- **预设路由**（`auto-transition.md` 的 hotfix/tweak 分派）：`_channel` 参数已随本次删除，若要实现需先定「谁来决定预设」。
