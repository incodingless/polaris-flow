...
> **2026-09-23 已落地**：本表的用词与两个提示语模板已收进 `zh/policies/auto-transition.md` 的
> 「压缩时机与恢复清单」一节，作为**唯一来源**；技能正文只保留自己的「读什么 / 从哪继续」，
> 并留指针指向该节。同时新增「恢复清单四件」（任务身份 / 进度依据 / 上一段产出 / 起始步骤），
> 明确 **不读 `state.yaml.phase`**（它是非权威镜像）。
