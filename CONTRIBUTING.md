...
Thank you for helping improve Polaris. This guide is meant to be practical: it
...
```bash
git clone https://github.com/rpamis/polaris
cd polaris
pnpm install
pnpm build
```
...
```bash
npx vitest run test/ts/polaris-scripts.test.ts
```
...
test: cover polaris state transitions
```
...
│   ├── init.ts        # polaris init
│   ├── status.ts      # polaris status
│   ├── doctor.ts      # polaris doctor
│   └── update.ts      # polaris update
...
Shell scripts live under `assets/skills/polaris/scripts/` and must work on macOS,
...
- Add new scripts to the `beforeEach` copy list in
  `test/ts/polaris-scripts.test.ts`.
- Add new scripts to `assets/manifest.json`.

Script dependencies:

```text
polaris-state.sh <- polaris-guard.sh, polaris-handoff.sh, polaris-archive.sh
polaris-yaml-validate.sh <- polaris-guard.sh (preflight phase)
polaris-handoff.sh <- polaris-state.sh (writes handoff_context/handoff_hash)
```
...
## `.polaris.yaml` State Changes

When changing fields in a `.polaris.yaml` state file, update all three places:

1. `assets/skills/polaris/scripts/polaris-state.sh` for the `cmd_set` whitelist and
   enum validation.
2. `assets/skills/polaris/scripts/polaris-yaml-validate.sh` for schema validation
   and `KNOWN_KEYS`.
3. `test/ts/polaris-scripts.test.ts` for YAML examples and assertions.
