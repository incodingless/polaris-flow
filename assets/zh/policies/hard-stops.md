...
| H12 | 任何对主仓 `.polaris/workflow.yaml` 的写操作（read-modify-write 全文重写）**必须**先按 `policies/workflow-lock.md` 获取 `.polaris/.locks/workflow.lock` 互斥锁；写后**必须**回读校验自己修改是否生效；锁内容含 `skill_name`+PID+启动时间；`trap` 自动释放；stale 阈值 5s（正常 RMW < 100ms）。ship 内嵌套使用时锁顺序固定为 ship.lock → workflow.lock，避免死锁 | `design`/`plan`/`ship`/`bugfix`/`hotfix` 中所有写 workflow.yaml 的位置 | 立即停止 |
...
| H14 | `[polaris-flow] 阻断:开发类路径(已选功能 = M01/M04/P01/P02/P03/M03)必须先走完 polaris:flow 零步,把 需求内容 填到非 待预检 且非 无(HARD STOP H14)。当前 需求内容=<值>,应停在 Step 0.1 识别 → 0.2 强制附需求 → 0.3 询问自动评估 → 0.4 自动路由,而不是直接加载技能。` |