...
| ID | 规则 | 适用 skill | 违规后果 |
|----|------|-----------|---------|
| H8 | 每次状态切换**必须**在回复中输出一行 `[polaris-flow] 进入阶段: <verb>` 或 `[polaris-flow] worktree: ...` 等可见状态行，让用户能可视化路径 | 所有 `/pofol:*` 命令对应的 skill 入口与关键步骤 | 重写当前回复 |
| H9 | `state.yaml` 中 `worktree.created_by_easy_flow=true` 时，ship 阶段**必须**在 `superpowers:finishing-a-development-branch` 完成后追加 **三段式**：(1) **harness 产物合回**（`.polaris/metrics/*` / `overrides.log` / `state.yaml` 终态 / `intention.md` 修订 → 主仓 `.polaris/` 顶层 + `.polaris/archive/<change_id>/`）；(2) git 合回（已合并则跳过）；(3) worktree 清理。**禁止**只 finishing 不清理 worktree；**禁止**未执行产物合回就 `git worktree remove`（会蒸发本次 reflect/审计数据） | `ship`（Step 3） | 立即停止 |
...
| H12 | 任何对主仓 `.polaris/workflow.yaml` 的写操作（read-modify-write 全文重写）**必须**先按 `policies/workflow-lock.md` 获取 `.polaris/.locks/workflow.lock` 互斥锁；写后**必须**回读校验自己修改是否生效；锁内容含 `skill_name`+PID+启动时间；`trap` 自动释放；stale 阈值 5s（正常 RMW < 100ms）。ship 内嵌套使用时锁顺序固定为 ship.lock → workflow.lock，避免死锁 | `design`/`propose`/`ship` 中所有写 workflow.yaml 的位置 | 立即停止 |
...
> 注：原 H3（强制 worktree 决策）已删除。worktree 创建改为非阻断式提示——由 `propose` skill 的 Step 1 在 `/pofol:propose` 入口询问用户即可，无需全局硬约束，无需独立的 worktree skill。
