...
```bash
TASK_IDS=$(bash "$PLUGIN_ROOT/scripts/workflow-entry.sh" get-active-changes --skill verify --repo-root "$REPO_ROOT" --phase verify)
RTID_EXIT=$?
```
...
| B | 用户确认后回 `/polaris-flow-build`（或回 design/plan，由用户选），更新设计与 specs |
...
4. 推进：
```bash
bash "$PLUGIN_ROOT/scripts/workflow-entry.sh" update-active --skill verify --where-change-id "$change_id" --set phase=delivery
```
