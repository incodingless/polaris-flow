...
> **执行入口**：本计划由 `/{{SKILL_NAME_PREFIX}}build` 通过 `/opsx:apply` 在 implementer subagent 内逐 task 执行。
...
|------------|----------------------|
| `/{{SKILL_NAME_PREFIX}}propose` | 按本模板生成**粗骨架**（可过 lint）；细粒度留给 plan |
| `/{{SKILL_NAME_PREFIX}}plan` | **覆写**为可执行细计划（Files + Interfaces + TDD/非TDD + 可复制命令）；跑 `tasks-lint`；再派 `plan-review-agent` |
...
| `/{{SKILL_NAME_PREFIX}}build`（implementer 跑 `/opsx:apply`） | 按 task 顺序逐条执行；通过 `<!-- TDD 任务 / 非 TDD 任务 -->` 决定子步骤节奏；apply 负责更新 checkbox |
