...
> **重要**：
> - **plan** 阶段调用 `/opsx:propose` 生成四件套前，必须 `read_file` 重读本文件（产出**粗骨架** tasks）。
> - **tasks** 阶段（`/polaris{{SKN_SPR}}coding{{SKN_SPR}}tasks`）在 design 完成后**覆写**同一路径的 `tasks.md` 为可执行细计划；每次覆写前必须再次 `read_file` 本文件。
> - 最终供 build / verify 消费的，以 **tasks 覆写后** 的版本为准。
...
## 模板正文（plan 初稿 / tasks 覆写须按此格式）

```markdown
# {{CHANGE_ID}} — 实施任务计划

> **执行入口**：本计划由 `/polaris{{SKN_SPR}}coding{{SKN_SPR}}build` 通过 `/opsx:apply` 在 implementer subagent 内逐 task 执行。
> **规划入口**：细计划由 `/polaris{{SKN_SPR}}coding{{SKN_SPR}}tasks` 按 writing-plans（骨架模式）覆写；plan 仅提供粗骨架。
...
## tasks.md 强制规则（plan 初稿与 tasks 覆写均必须遵守）
...
- **Interfaces**（tasks 覆写后强制；plan 初稿鼓励填写）：Consumes / Produces 写清后续任务依赖的符号或路径约定
...
## 与 plan / plan / lock / build 的契约

| 上游消费方 | 期待 tasks.md 提供什么 |
|------------|----------------------|
| `/polaris-flow-plan` | 按本模板生成**粗骨架**（可过 lint）；细粒度留给 plan |
| `/polaris-flow-tasks` | **覆写**为可执行细计划（Files + Interfaces + TDD/非TDD + 可复制命令）；跑 `tasks-lint`；再派 `tasks-review-agent` |
| `tasks-review-agent`（由 plan 派发） | 评审粒度、依赖排序、可执行性、测试缺口；写入 `reviews/tasks-review-report.md`，**评审期间不直接改 tasks**；消化与改写由 plan skill 完成；可选 Outside Voice（`openspec-review-agent`） |
