...
|------------|----------------------|
| `/polaris{{SKN_SPR}}coding{{SKN_SPR}}plan` | 按本模板生成**粗骨架**（可过 lint）；细粒度留给 plan |
| `/polaris{{SKN_SPR}}coding{{SKN_SPR}}tasks` | **覆写**为可执行细计划（Files + Interfaces + TDD/非TDD + 可复制命令）；跑 `tasks-lint`；再派 `tasks-review-agent` |
