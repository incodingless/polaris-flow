...
> 由 `delivery` skill 的 Step 3.4.5 调用。**实际执行由脚本 `hooks/polaris-sync.sh` 确定性完成**——主代理只负责调用脚本 + 处理 exit 2 的用户交互,**禁止**在脚本之外做任何额外的文件拷贝动作。

## 脚本位置

```bash
bash "$PLUGIN_ROOT/hooks/polaris-sync.sh" <worktree_path> <origin_repo> <change_id> [--overwrite|--suffix|--skip]
```
...
| `3` | worktree `.polaris/` 不存在 | `harness_sync = "skipped_no_source"`;跳到 3.5 |
...
`metrics_files` = 合回到**顶层** `.polaris/metrics/` 的文件数(archive 不存 metrics 副本——单 change 追溯通过 JSON 内 `change_id` 字段从顶层查询)。

主代理用此 JSON 输出状态行:
```
[polaris-flow] polaris 产物合回完成：metrics=3 files → 顶层 / overrides=12 lines / state.yaml + pre_design → archive/<change_id>/
```
...
```
.polaris/archive/<change_id>/
  state.yaml
  pre_design.md
  overrides.log
```

不含 `metrics/` 子目录——metrics 只存顶层 `.polaris/metrics/`。
