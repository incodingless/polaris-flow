...
# 0=成功 / 1=rebase 冲突（worktree 留中间态；用户手动 git rebase --continue 后重跑 /polaris-flow-delivery）
...
- `delivery.harness_sync: skipped_worktree_retained`（字段名沿用模板；语义=产物未合回）
...
| 结果 | 处理 |
|------|------|
| 成功（含 warnings） | `delivery.archive=archived`，写入 `archive_path`；warnings 追加到 Step 6 摘要 |
| exit ≠ 0 | **不做归档**：保持 `openspec/changes/<change_id>/` 原位；`delivery.archive=failed`，写入 `archive_error`；摘要注明失败原因。不阻断交付收尾 |

无论成功或失败，进入 Step 6（含 6.1）。用户若要事后补归档，可手动 `openspec-cn archive <change_id>`。

#### 5.3 用户选 B / C

- **B** → `delivery.archive=deferred`；提示稍后手动 `openspec-cn archive <change_id>` 或 `/opsx:archive`
- **C** → `delivery.archive=skipped`
...
**必做**（含 `delivery.archive=failed`：OpenSpec 目录仍在原位，仅清 polaris 游标与 tasks 档案）。
...
重载：`change_id`、`worktree_path`、`verify.*`（status / blocked / overall_score）、`delivery.*`、`worktree.*`、本 skill 停在哪一步。
