---
name: propose
description: "用户触发 /propose 或要求基于 intention.md 生成 OpenSpec 四件套时必须使用本 skill。
...
<HARD-GATE>
- **禁止**未确认 `intention.md` 的存在且未读取其内容就调用 `/opsx:propose`
...
**1.1 前置自检**：读 `.polaris/tasks/<change_id>/state.yaml: worktree.created_by_easy_flow`。已存在非空值 → 跳过本步，输出 `[polarisd-flow] worktree: 已决策（<...>），跳过本次询问。` 后进入 Step 2。

**1.2 询问用户**（统一文案，不按 tier 区分）：

> 即将进入 propose 阶段，会生成OpenSpec 规格文档并改动仓库。是否为本次变更建立独立的 git worktree？
>
> A. 是，创建 worktree
> B. 否，留在当前工作目录
...
bash "$PLUGIN_ROOT/hooks/workflow-entry.sh" update-active --skill propose \
  --where-task-id "$task_id" --set phase=propose --set worktree-path="$target_path"
```
...
不动 git。更新主仓 `.polaris/tasks/<task_id>/state.yaml`：写 `worktree.created_by_polaris_flow: false`、`current_verb: propose`。同步 workflow.yaml(phase 切到 propose,worktree_path 仍为空):

```bash
bash "$PLUGIN_ROOT/hooks/workflow-entry.sh" update-active --skill propose \
  --where-task-id "$task_id" --set phase=propose
```
...
**2.1 定位**：worktree 模式（1.3.A）下取 `<target_path>/.polaris/tasks/<task_id>/intention.md`；主仓模式（1.3.B）下取 `<main_repo_root>/.polaris/tasks/<task_id>/intention.md`。

**2.2 文件存在性 + 完整性**：

| 情况 | 处理 |
|---|---|
| **文件不存在** | fallback：把用户调用 `/polaris-flow:propose` 时的原始消息作为 propose 输入；输出 `[polaris-flow] 未找到 intention.md，使用用户原始 prompt 作为 propose 输入。` 后跳到 Step 3.2 |
...
**3.2 组装输入**：完整路径 → `intention.md` 全文整段嵌入（不要总结）；fallback 路径 → 用户调用时的原始消息。

**`intention.md` 节 → OpenSpec 四件套映射**（仅完整路径适用）：

| `intention.md` 节 | 写入位置 | 要求 |
|---|---|
| `## Reframe 历程` | `proposal.md` 的 "Why / Context" 节 | 必须包含 |
| `## Constitution Alignment` | `design.md` 的 `## Constitution Alignment` 节（逐条对齐 Core Principle） | 必须包含 |
| `## Premises` | `design.md` 的 `## Premises` 节 | 必须包含 |
| `## Decisions`（架构 + 技术选型） | `design.md` 的 `## Decision` / `## Architecture` 节 | 必须包含架构决策、方案选型（深度技术设计在 design 阶段 Design Doc 细化）|
| `## Alternatives` | `design.md` 的 `## Alternatives` 节 | 必须包含 |
| `## 任务范围 / Scope` | `tasks.md` 任务划分的范围依据 | 必须包含，任务为粗粒度任务，每个任务20分钟左右|
| `## Open Questions` | `proposal.md` 的 `## Open Questions` 节 | 必须包含 |
...
按以下标准产物循环逐个生成 `proposal`、`design`、`tasks`：
...
1. 四件套均生成：`openspec/changes/<change_id>/` 下含 `proposal.md` / `design.md` / `specs/` / `tasks.md`，
   逐个确认这4项产物的文件路径存在且非空。任一文件缺失或为空时，不得进入Step 5 或执行阶段守卫，必须回到创建步骤补充。
2. `proposal.md` 含问题背景、目标、范围、非目标
3. `design.md` 含高层架构决策、方案选型、数据流：
  - `## Constitution Alignment` 节，且逐条覆盖 Core Principle
  - `design.md` 含 `## Alternatives` 节，逐条说明未选方案及拒绝理由
  - `design.md` 含 `## Premises` 节
