...
| `degradation` | 动作 |
|---------------|------|
| `null` | 从 `agents` 选一项：路径型 → 记 `dispatch=path` + `subagent_path`；builtin → 记 `dispatch=builtin` + `subagent_type=id`。多项时按 decision-point 让用户选，或取第一项并告知用户 |
| `empty` | 记 `dispatch=default`（宿主默认 subagent，不绑 path） |
| `inline` / `unsupported` | **强制**改为本会话 inline 执行 apply；输出原因；进入 Step 3.3（不违反 HARD-GATE 例外） |
