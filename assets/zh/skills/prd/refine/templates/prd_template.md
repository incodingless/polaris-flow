# {{系统/产品名称}} · {{需求中文名}} — 产品需求文档（PRD）

> **模板版本**：prd-template v2.0
> **文档状态**：[草案 / 评审中 / 已定稿]
> **需求中文名**：{{需求中文名，4-12 汉字，与 state.yaml 的 naming.req_name_cn 一致}}
> **需求编号前缀**：{{前缀，大写 ASCII 2-16 位，全文编号的命名空间}}
> **文档编号**：{{PREFIX}}-PRD-V1.0（PREFIX 为需求编号前缀，如 UAP）
> **当前版本**：{{V1.0}}
> **编写人**：{{姓名}}
> **所属系统/模块**：{{系统名 / 模块名}}
...
2. **规则可追溯**：每条业务规则分配唯一规则编码（`{{PREFIX}}-BR-XXX`），测试与缺陷单引用该编码。
3. **编号命名空间**：全文编号统一以「需求编号前缀」开头——模块 `{{PREFIX}}-F01`、功能点 `{{PREFIX}}-F01-01`、业务规则 `{{PREFIX}}-BR-101`；分支 `B-01`、验收 `AC-1` 为功能点内局部编号不带前缀。**禁止出现无前缀的裸编号**，否则跨需求无法定位。
4. **单一事实源**：同一条规则只在「第 6 章 业务规则汇总」定义一次，功能点内只引用编码，避免多处表述冲突。
5. **异常必须闭环**：每个功能点至少覆盖正常、异常、边界三类场景；7 类标准异常（网络 / 服务 / 权限 / 数据 / 并发 / 第三方依赖 / 业务状态）逐条自查。
6. **图表用 Mermaid**：流程图用 `flowchart`，实体关系用 `erDiagram`，状态流转用 `stateDiagram-v2`，时序用 `sequenceDiagram`。图后必须配文字或表格说明，图不替代规则。
7. **产品语言**：除第 4 章数据模型允许字段名等技术细节外，其余章节不得出现技术实现细节（具体表结构、框架、中间件选型）。
8. **章节裁剪**：小型需求可裁剪 `[可选]` 与未命中的 `[条件必填]` 章节，但**第 1、5、10 章不可裁剪**；裁剪处保留标题并注明「本次不适用」。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| {{US-01}} | 用户故事 / 基线 / 工单 | {{作为 XX 角色，我希望…}} | {{5.2.1}} | {{PREFIX}}-F01-01 | {{PREFIX}}-F01-01 AC-3 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 网络 | {{请求超时/断连}} | {{哪一步检测}} | {{重试策略、提示文案}} | {{重新提交}} | {{PREFIX}}-F01-01 |
| 服务 | {{下游不可用/降级}} | | | | |
| 权限 | {{越权访问/数据越界}} | | | | |
| 数据 | {{重复提交/脏数据/超长}} | | | | |
| 并发 | {{同时操作同一对象/库存超卖}} | | | | |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| IN_USE | 发起回收 | RECYCLING | {{载体管理员}} | {{PREFIX}}-BR-101 | {{写入生命周期记录}} |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| {{PREFIX}}-F01-01 | {{载体台账}} | {{载体新增}} | {{一句话说明用途}} | {{管理员}} | P0 | 用户主动 | {{US-01}} |
| {{PREFIX}}-F01-02 | {{载体台账}} | {{台账查询}} | | {{全部}} | P0 | 用户主动 | {{US-02}} |

### 5.2 功能模块详述

#### 5.2.1 [{{PREFIX}}-F01] {{模块名称}}
...
1. {{PREFIX}}-BR-101：仅主载体状态为「在用」时允许发起流转
2. {{PREFIX}}-BR-102：所有流转操作须经审批，审批结果异步回写
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| {{PREFIX}}-BR-101 | 校验 / 计算 / 状态流转 / 权限 | {{明确无歧义的规则说明}} | {{哪个步骤}} | {{阻止流程 / 警告提示 / 自动处理}} | {{B-01}} |
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| {{PREFIX}}-BR-101 | 操作限制 | {{终态载体禁止发起任何流转操作}} | 全模块 | 流转发起校验 | 阻止流程 | {{5.2.1}} |
| {{PREFIX}}-BR-201 | 状态流转 | {{主载体状态由子载体状态自动聚合，禁止人工写入}} | 全模块 | 状态变更时 | 自动处理 | {{7.2}} |
| {{PREFIX}}-BR-301 | 数据隔离 | {{主/子载体与生命周期记录均不提供物理删除接口}} | 数据层 | 删除请求 | 阻止流程 | {{4.4}} |

**自检**：□ 规则间无冲突　□ 每条规则可量化判定　□ 功能点内未重复定义　□ 编码全局唯一　□ 全部编码带统一编号前缀
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| {{Q-01}} | {{局部销毁后主载体状态如何聚合}} | {{状态机}} | {{PREFIX}}-F02 | {{YYYY-MM-DD}} | {{姓名}} | 阻塞 | {{按剩余子载体聚合}} |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| {{US-01}} | {{…}} | {{5.2.1}} | {{PREFIX}}-F01-01 | {{AC-1, AC-3}} | 已覆盖 |
