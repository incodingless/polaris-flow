...
| 对象 | 格式 | 示例（前缀 `UAP`） |
|---|---|---|
| 功能模块 | `{前缀}-F{dd}` | `UAP-F01` |
| 功能点 | `{前缀}-F{dd}-{dd}` | `UAP-F01-01` |
| 业务规则 | `{前缀}-BR-{ddd}` | `UAP-BR-101` |
| 上游锚点（继承初稿） | `{前缀小写}-cap-001` | `uap-cap-001` |
| 文档编号（终稿头部） | `{前缀}-PRD-{最终版本号}` | `UAP-PRD-V1.2` |
| 交付文档名（ship 迁移后） | `{前缀}-{中文名}-需求终稿-{最终版本号}.md` | `UAP-用户权限精细化-需求终稿-v1.2.md` |

> 分支编号 `B-01`、验收编号 `AC-1` 为功能点内局部编号，不加前缀。初稿继承的锚点若与前缀不一致，须在本阶段统一改写并同步修正引用。
...
- **5.2 功能点业务规则 → 第六章**：功能点内只引用 `{前缀}-BR-XXX` 编码，禁止重复定义规则；基线第 6 章的 `R001` 与终稿 `{前缀}-BR-101` 的映射写入追溯矩阵
...
> 需求中文名：{req_name_cn}
> 需求编号前缀：{req_prefix}
> 文档编号：{req_prefix}-PRD-{最终版本号}（评审迭代后的实际版本，取正文「当前版本」字段值，如 V1.0 / V1.2）
> 基线版本：{Baseline版本标识}
> 评审状态：已通过完整评审，阻塞问题清零
...
| 存在 `full_review_report.md` 且含未清零的 P0/T0 | 从 Step 3.3 修复开始（直接在 `prd_final_draft.md` 上修复并递增版本号） |
| `full_review_report.md` 阻塞问题已清零，无 `prd-final-v1.0.md` | 从 Step 4 定稿开始 |

- 已完成的评审结果保留有效，无需重复执行
- 阻塞问题修复记录完整保留，从中断处继续迭代
- 评审-修复阶段的续跑位置以 `prd_final_draft.md` 的「当前版本」与「修订记录」为进度依据（合并后全文是唯一真源）
