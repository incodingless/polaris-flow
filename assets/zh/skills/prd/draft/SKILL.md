...
> 执行铁则：
> 1. 一章完成（生成-自检-写入磁盘-用户确认）之后，才允许进入下一章；
> 2. 不允许一次性批量生成多章节；
> 3. 用户修改意见：用增量编辑修改已落盘文件（只改差异片段，不整章重写），再次展示【摘要+校验表】，确认后才标记该章为 confirmed；
> 4. 章节全文只允许写入 sessions 下独立 md 文件一次，禁止先在会话全文展示、再全文落盘造成的二次输出。
