...
执行步骤：
1. 完整读取《需求基线》；
2. 解析提取实体：角色、业务场景、通用能力、业务规则，分配锚点 ID：`cap-xxx`（通用能力）/ `scene-xxx`（业务场景）；构建索引 `baseline_index.json`；锚点 ID 一律小写 + ASCII 连字符 `-`，写入文档时须用显式 HTML 锚点 `<a id="cap-xxx"></a>` 定义目标（规范见 `./policies/writting-rules.md`）；
...
2. 锚点完整性校验：检查每个锚点引用 `[xxx](#cap-xxx)` / `[xxx](#scene-xxx)` 是否存在对应的 `<a id="cap-xxx">` / `<a id="scene-xxx">` 目标定义；引用存在但目标缺失、或 ID 大小写/连字符不一致的，标记异常；
...
步骤：
1. 按章节顺序合并全部 session 文件，自动生成 Markdown 目录，修正全文锚点跳转：确保每个锚点引用都有对应 `<a id="cap-xxx">` / `<a id="scene-xxx">` 目标定义，且 ID 小写、ASCII 连字符 `-`、与引用完全一致；文档头部统一写入「评审重点指引」与「图表索引」（F08）；
2. 全局质量自检三件套：
   1）清理残留技术术语；
