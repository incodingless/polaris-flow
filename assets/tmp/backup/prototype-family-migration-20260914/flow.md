...
| **R11** 由需求文档生成原型 | `polaris{{SKN_SPR}}prd{{SKN_SPR}}prototype` | prototype **建造**：9 阶段（1–8 判断层：需求理解 → 黄金任务流 → IA → 页面体系 → 页面模式 → 页面结构 → 交互状态 → 视觉系统；**9 执行层**：9.1 实现 → 9.2 验证 → **9.3 评审 → 调用 R12** → 9.4 交接），产出 6+1 交付物（均落盘）；**9.3 是交付前门禁**——评审结论为「不得交付」即停在 9.3，不得进 9.4 |
