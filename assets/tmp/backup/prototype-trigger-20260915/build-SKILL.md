...
| 原型 + `flow.json` + `handoff.md` 齐备 | 已交接 | 提示本环节已完成；推进到 `polaris{{SKN_SPR}}prototype{{SKN_SPR}}ship`（评审 → 确认 → 归档） |
...
推荐顺序：`scaffold init` → `verify --layer=1` 取静态基线（骨架应即刻全绿）→ 填页面主体 → `verify --flow=… --strict` 全三层 → 移交 `ship`（flow.md **R11** 制作链的收尾段）。
...
**交付门禁不在本技能内**：本环节结束后推进到 `polaris{{SKN_SPR}}prototype{{SKN_SPR}}ship`——由它在独立上下文派发 `review` 做评审，确认后归档并收尾任务（flow.md **R11** 的制作链）。
评审结论为「不得交付」时回本环节修复（→ 重跑 Step 4.2 → 重新交付），**P0 未清零不得对外交接**。
...
5. 交接材料已备齐并移交 `polaris{{SKN_SPR}}prototype{{SKN_SPR}}ship`（评审结论不属本技能的退出条件）
...
（13）原型骨架是否由 `scripts/scaffold.mjs` 生成（而非手写整份大文件后再做破坏性编辑）；页面与图标的新增是否走 `add-page` / `add-icon` 的定点插入（命令见 `Step 4.1`）；
（14）Token 取值是否来自正确的来源：需求方提供了设计系统 / 视觉规范时，是否已用它覆盖（`--tokens`），而不是留着技能兜底基线？换过规范后是否重跑了 `verify`（换规范后对比度仍须 ≥4.5:1，阈值与取值来源无关；`set-tokens` 用法见 `Step 4.1`）；
（15）`verify.mjs --strict` 是否已跑出退出码 0（**命令与退出码表见 `Step 4.2`**）：静态检查 → 逐页冒烟 → 黄金流链路 + 三档分辨率实测，三层一层都不能跳。退出码非 0 不得交付；脚本报 WARN 的项需人工判断；演示数据去个人化已并入脚本，无需另行人工扫描。**脚本过了只说明 Step 4.2 完成——交付门禁是 `ship` 派发的评审结论，不是脚本退出码**；

（16）交付物形态是否对照 `example/deliverables.md` 校准过（6+1 交付物各自是否到位，而不是只有一份原型）；涉及页面机制、Token 命名契约或脚本判定的改动，是否已重跑 `evals --selftest` 并全绿（见上文「样例与评测」）。
