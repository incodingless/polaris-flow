...
**阶段 1｜需求理解与产品快速定义**：先出《Web 原型任务理解卡》，它至少包含：
...
**阶段 2｜黄金任务流 Golden Flow**：必须找到**最能体现产品价值的一条完整任务闭环**。
...
**阶段 3｜信息架构 IA**：先设计产品结构，不直接画页面。IA 原则：高频业务优先；一级导航控制数量；
...
**阶段 4｜页面体系与职责划分**：**不设页面数量的机械上限或下限**，数量由 **用户任务 + 业务闭环 + 信息架构 + 页面职责 + 交互复杂度** 自然推导——不得为「页面少」强行合并本应独立的复杂任务，也不得为「系统看起来完整」创建没有明确用户任务的页面。
...
**阶段 5｜页面模式选择**：不得所有页面都用「查询 + 表格 + 弹窗」，按任务自动选模式。九种模式：
