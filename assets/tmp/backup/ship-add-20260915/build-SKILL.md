...
（15）`verify.mjs --strict` 是否已跑出退出码 0（**命令与退出码表见 `Step 4.2`**）：静态检查 → 逐页冒烟 → 黄金流链路 + 三档分辨率实测，三层一层都不能跳。退出码非 0 不得交付；脚本报 WARN 的项需人工判断；演示数据去个人化已并入脚本，无需另行人工扫描。**脚本过了只说明 Step 4.2 完成——交付门禁是 flow.md R13 的评审结论，不是脚本退出码**；
