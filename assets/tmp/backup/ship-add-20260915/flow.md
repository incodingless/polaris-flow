...
- `已选功能`：P01–P03 / M01–M03 / R01–R03 / R11–R13 / T01–T02，其中之一
...
**触发条件**：本命令被调用即启动零步；`已选类别` 或 `已选功能` 落在开发类（P01 / P02 / P03 / M01 / M03）时**必须走完**，其他类别（需求 / 测试 / 维护的非开发项）可直接跳过。R11–R13 虽同为产物生成路径，但其需求输入由《需求类与测试类选项的前置依赖》逐条把关，且 0.3 / 0.4 的复杂度自动路由对原型无意义，故**不**进零步。
...
```json
{
  "questions": [{
    ...
    "options": [
      ...
      { "label": "R11 · 生成原型制作思路", "description": "照需求产出《原型蓝图》：任务理解卡 / 黄金流 / IA / 页面清单，人工确认后冻结" },
      { "label": "R12 · 按蓝图制作原型", "description": "照已确认的蓝图产出高保真可交互 HTML 原型并自行验证" },
      { "label": "R13 · 评审已有原型", "description": "对照需求与已确认蓝图做质量审查，产出问题清单与整改建议" }
    ]
  }]
}
```
...
| **R11** 制作原型 | `polaris{{SKN_SPR}}prototype{{SKN_SPR}}blueprint` | prototype **蓝图**：需求理解 → 黄金任务流 → 信息架构 → 页面体系与职责 → **人工确认 → 冻结**；产出《原型蓝图》（`blueprint.md` + `task-card.md` / `golden-flow.md` / `ia.md` / `page-list.md`，均落盘）。**确认是硬门禁**——未确认不得进 R12；确认后蓝图冻结，后续发现缺口标记 `WEB-DESIGN-CHANGE-XXX` 回报，不得自行改写 |
| **R12** 按蓝图制作原型 | `polaris{{SKN_SPR}}prototype{{SKN_SPR}}build` | prototype **建造**（前置：R11 蓝图已确认）：页面模式 → 页面结构 → 交互与状态 → 视觉系统 → 实现（`scaffold.mjs`）→ 三层验证（`verify.mjs`，退出码 0 才算过）→ 交接；产出原型 + `flow.json` + 页面结构与状态说明 + 研发交接说明。原型一律落 `$REPO_ROOT/.polaris/tasks/$task_id/`；发现蓝图缺口标记 `WEB-DESIGN-CHANGE-XXX` 回报，**不自行改蓝图** |
| **R13** 评审已有原型 | `polaris{{SKN_SPR}}prototype{{SKN_SPR}}review` | prototype **评审**（**只评不改**）：实际执行三层验证 + 五维质量审查 + 一票否决 31 条 → 结论（可交付 / 修复后可交付 / 不得交付）+ P0–P3 分级问题清单与整改建议；**不从第一步重做**。判据（`§33` 五维 / `§34` 一票否决 / `§36` 最终判断标准）的唯一来源是该技能自己的 `references/01-quality-criteria.md`；三层验证执行体 `scripts/verify.mjs` 在 `polaris{{SKN_SPR}}prototype{{SKN_SPR}}build`。结论为「不得交付」时**回 R12 修复后重评审** |
...
`需求内容` 字段名沿用至 tweak / normal / specify / hotfix / prototype（蓝图 / 建造），被加载技能按以下约定读取：
...
- prototype（**R11 蓝图** / **R12 建造**）：作为「最低输入 6 项 + 可选增强输入」的来源——需求文档已能判断的用户 / 场景 / 一期范围 / 页面功能 / 核心流程不重复询问（`blueprint` 技能的 `references/02 §4.3`）
- prototype-review（**R13 评审**）：不消费 `需求内容` 作为评审判据，只消费 `附加上下文` 里的原型文件（与可选的需求文档、已确认蓝图）；缺需求文档时按该技能 §三 的「无需求文档取证法」判，不得凭空补需求
...
| **R11** 生成原型制作思路 | 需可读的需求输入：需求基线 / 定稿 PRD / 建设方案 / Agent 设计文档（任一并可被 `references/02 §4` 映射为用户 × 场景 × 任务） | 提示用户先执行 **R01 / R02**，或在第三步把方案文档附进 `附加上下文` |
| **R12** 按蓝图制作原型 | 需 **R11 已确认**的《原型蓝图》（`blueprint.md` + `page-list.md`） | **阻断**：提示用户先执行 **R11** 并完成人工确认。蓝图未确认不得开工——制作环节不自行补设计结论 |
| **R13** 评审已有原型 | 需待评审的原型文件（`.html`），**必须**出现在 `附加上下文` 里；需求文档与已确认蓝图为可选但强烈建议（蓝图是取证源 **E2**，能把「是否符合已确认设计」判到确定结论） | 提示用户在第三步附上原型文件路径；有黄金流 JSON 一并附上则链路审查更完整（`verify.mjs --flow=`）；无需求文档不阻断，业务维度按评审技能 §三 的「无需求文档取证法」判 |
