...
- **需求类菜单（2026-09-15 起 5 项）**：R01 用户需求(discovery) / R02 产品需求 / R03 就绪度(readiness) /
  **R11 制作原型** / **R12 评审已有原型**。（原 R04/R05 编号已废弃。）
- **原型只暴露两个菜单入口（2026-09-15 定）**：R11 = `blueprint` → `build` → **`ship`** 全链（build / ship **不占菜单项**）；
  R12 = `review` 单独评审（只评不改、**不归档**）。原 R12(build) 删除、原 R13 重命名 R12。
  改菜单要动 6 处：菜单 JSON / `已选功能` 行 / 零步说明 / 阶段链表 / 需求内容消费表 / 前置依赖表。
- **prototype 族四技能**：`blueprint`（需求理解→蓝图·人工确认冻结）/ `build`（按蓝图做原型 + `verify.mjs` 三层验证）/
  `review`（判据唯一来源 §33/§34/§36 · 只评不改）/ **`ship`（派发独立评审 → 人工确认 → 归档 → 任务收尾）**。
  **ship 存在的首要理由是独立性**：review 入口 B 原由建造者同会话执行，只能"如实标注独立性受限"；
  ship 用 `subagent-probe → 选 agent → subagent-dispatch` 派发后评审跑在独立上下文，该限制不成立；
  平台不支持 subagent 时降级 inline 并在报告标注「本轮未独立执行」，**判据不放宽**。
- **触发语义分层（2026-09-15 定）**：`blueprint` 吃**总目标语义**（做个原型 / 从需求到原型交付 / 这个需求要做哪些页面），
  `build` 吃**接续语义**（照蓝图做 / 蓝图确认了开始做 / 继续做 / 重做某几个页面 / **修改已有原型**）。
  不能让两家同吃"制作原型"——路由会靠模型临场猜。**触发词分不开的意图交给状态分**：
  blueprint Step 1.5 有「`blueprint.status == confirmed` → 不再重跑，引导到 build」分支。
- **build 完成后暂停等确认（2026-09-15 定）**：不自动推进 ship，呈现「已完成 + 可交付」后由用户决定，
  避免原型没被看就归档。同一口径散在 build 的 4 处（Step 5 正文 / Step 1.5 进度表 / 推荐顺序 / 退出条件#5），改一处不算完。
- **改已有原型归 build**，且无蓝图时**不阻断**：build Step 2.1 门禁表有专用分支（有落盘 `.html` 无蓝图 →
  限于用户指定点最小改动，走 scaffold 定点插入，不整份重写，并提示蓝图缺失削弱范围判据）。
  **归口一个语义时必须同时检查入口门禁能不能过**，否则等于接住了却进不去。
...
- **SKILL.md 尾部章节统一叫「参考文件与工具」/「交付前自检」，不叫「附录 A/B」**（2026-09-15 定）：
  blueprint 与 build 同名，review 叫「参考文件（按需加载）」，全仓 SKILL.md 层「附录」已清零
  （prd 模板里的"附录"是产物章节名，与本约定无关）。改章节名必须同步 ref00 / ref05 / 跨技能引用。
- **执行体（脚本命令）必须就近放进调用它的 Step，不能堆在尾部**：放尾部 = 模型执行到关键一步要跳读 40 行，
  且必然与自检清单重复成双源。尾部只留「索引类」内容（读哪份文档 / 什么时候读 / 样例与评测），
  索引删了模型就不知道该读什么——**所以尾部可以瘦身，但不能清空**。
- **尾部章节没有 Step 指过去 = 死内容**（2026-09-15 两次踩到）：脚本命令放附录、Step 4 够不着；
  「交付前自检 16 条」挂在文末、build 自己零引用（只有 review 偶尔当取证线索）。
  **改完 Step 后要反问一句：文末还剩什么？谁会读它？** 处置 = 给显式调用点 + 减重
  （脚本与前置 Step 已判的不重复人工判；减重前须确认别处确有覆盖，不是丢规则）。
  同一判据的第二个变体（同日第三次踩到）：**「要读的规范」没有触发点 = 没人读**，且**被尾部索引表兜住时
  完全看不出来**——表在，文件看起来"被引用了"，实际没有 Step 会打开它。2026-09-15 查出 build 的
  `references/04`（P0 页面 12 项 / 11 种状态）、`06`（多分辨率验收矩阵）、`07`（三层验证规格 / 研发交接 /
  6+1 交付物）在 Step 0–6 **零引用**，尾部表是唯一入口；Step 3.2/3.3 明明在做 04 里定义的事却只指了 05。
...
- **改技能后按「同一个 bug 的副本」扫一遍**，`Step x.y` 这类指针常在 3–4 个文件里各写一份：
  2026-09-15 的 `Step 2.3`（实际不存在，只有 2.1/2.2）就同时写在 SKILL.md 表、Step 2.1 结论、
  ref00 头、ref01 头四处。**拼写类 bug 也要当真 bug 看**：`buleprint` 让提示里的技能名指向不存在的技能，
  且 `state.yaml` 的 `buleprint.status` 与 blueprint 写入的 `blueprint.status` 对不上，门禁永远过不了。
- **长会话里用户可能自己用编辑器改同一个文件**：2026-09-15 加 ship 时，`flow.md` 的 R11 标签在两次读取之间
  被从「生成原型制作思路」改成「制作原型」（描述没动）。**每轮 Edit 前重新 Read 目标行**，
  别拿上一轮 grep 的快照当 old_string——Edit 报"字符串不存在"就是信号，先看 mtime 与当前内容。
- **多行 old_string 替换要数清行数**：同一次改前置依赖表，old 是三行（R11/R12/R13）、new 只写一行，
  R13 评审行被整行吞掉，事后才发现补回。**new 的行集合必须逐一对应 old 的每一行。**
